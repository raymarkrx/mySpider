...
def getList(url,aim_dir):
    ...
    for topic_url in links:
        print multiprocessing.current_process().name +"==帖子页面=="+topic_url
...
def do(init_url):
    ...
    for topics_url in url_pages:
        pool.apply_async(doIt, (topics_url,aim_dir, )) 
    pool.close()
    pool.join()
